module.exports.config = {
 ...
 module.exports.handleEvent = async ({ Threads, api, event}) => {
   ...
   const thoi_gian_spam = 50; // 60000 millisecond (1 phút)
 	...
 	if ((global.client.autobanthread[threadID].timeStart + thoi_gian_spam) <= Date.now()) {
 	  ...
 	  if (global.client.autobanthread[threadID].number >= so_lan_spam) {
 	    const time = moment.tz("Asia/Ho_Chi_minh").format("DD/MM/YYYY HH:mm:ss");
 			...
 			data.banned = true;
 			data.reason = `spam bot ${so_lan_spam} lần/${thoi_gian_spam/50}phút`;
 			data.dateAdded = time;
 	      ...
 	      number: 0
 			...
 			api.sendMessage({
 			  body: `🐙 ${threadID} \n ${dataThread.threadInfo.threadName}\n> Nhóm đã bị cấm sử dụng bot \n🐙 lý do: spam bot ${so_lan_spam}lần/${thoi_gian_spam/50}phút\n> \n🐙 Unban sau ${Math.floor(unbanAfter/60000)}phút ,hẹn gặp lại \n<bye>\ib Admin Xthanh để được unban sớm nhất : https://www.facebook.com/thanhthongthai2oo3`,
