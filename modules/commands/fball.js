...
module.exports.handleReply = async function({
	...
	var coinMrandom = Math.floor(Math.random() * 10000) + 100;
	///////////
	...
	switch (handleReply.type) {
		case "choosee": {
			switch (event.body) {
				case "1":
				case "-m": {
					msg = `(1/-goiRandomMcoin) [ GÓI RANDOM MCOIN ]\nSố tiền bỏ ra để thực hiện giao dịch là [ 10000Coin ] có cơ hội nhận được phần thưởng [ 100000Coin ]`;
					return api.sendMessage({
						body: msg
					}, threadID, (error, info) => {
						global.client.handleReply.push({
	...
	switch (handleReply.type) {
		case "Market": {
			switch (event.body) {
				case "1":
				case "-goiRandomMcoin": {
					msg = `⚡Giao dịch thành công tài khoản của bạn bị -[ 10000Coin ]\nBạn mở random được [ ${coinMrandom}mCoin ]`;
	...
	switch (handleReply.type) {
		case "vMFG": {
			switch (event.body) {
				case "1":
				case "-xC": {
					msg = `⚡Số Coin của bạn là: [ ${money}Coin ]`;
					return api.sendMessage({
						body: msg
					}, threadID, messageID)
				}
...
const reply = function (reply){
  return api.sendMessage(reply, threadID,messageID)
					...
					//
					reply(`Bạn Mất [ 50000Coin] Khi Quay`);await Currencies.decreaseMoney(senderID, 50000);
					//quay
      reply(`[ Đang Quay...... ]`);
